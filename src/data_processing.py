...
class DataProcessor:
    ...
    def filter_users(self, min_rating = 400):
        try:
            ...
            rating_df = self.rating_df[self.rating_df["user_id"].isin(n_ratings[n_ratings>=400].index)].copy()
            logger.info("Filer users complete")

        except Exception as e:
            raise CustomException("Failed to filer frequent users", sys)
    ...
    def scale_ratings(self):
        try:
            ...
            logger.info("Scaling done for ratings")

        except Exception as e:
            raise CustomException("Failed to scaling the ratings", sys)
    ...
    def encode_data(self):
        try:
            ...
            raise CustomException("Failed to encode", sys)
        

    def split_data(self, test_size = 1000, random_state = 43):
        try:
            # Data randomized
            self.rating_df = self.rating_df.sample(frac=1,random_state=43).reset_index(drop=True)   #frac = 1 means shuffeling whole data

            X = self.rating_df[["user","anime"]].values
            y = self.rating_df["rating"]

            train_indices = self.rating_df.shape[0] - test_size   #Since the whole data's already randomized across rows
            ...
            logger.info("Data randomized and splitted succesfully")
    ...
    def save_artifacts(self):
        # Arrays cannot be saved as a csv, need joblib to save as pkl
        try:
            artifacts = {
            ...
            for name, data in artifacts.items():
                ...
                logger.info(f"{name} saved into processed dirctory") 
            ...
            self.rating_df.to_csv(RATING_DF, index = False)  # no need to joblib saving dataframe

            logger.info("All data processing artifacts saved")
    ...
    def process_anime_data(self):
        try:
            ...
            df.iloc[0, 1] = 'DVC Testing, version 2'

            df.to_csv(DF_PATH, index = False)
            synopsis_df.to_csv(SYNOPTIS_DF, index = False)
            logger.info("DF and SYNOPSIS DF saved succesfully ...")
...
if __name__ == "__main__":  #"As soon as someone runs this file"
    data_processor = DataProcessor(
        input_file = ANIMELIST_CSV, 
